...
import { Icon, getAnnotationsName, type ApplicationsInstanceDetail } from '@ks-console/shared'
...
export function OPAppEditModal({
  ...
  return (
    <Modal
      width={691}
      visible={visible}
      title={<Field value={t('APP_STORE_LONG_NAME_DESC')} avatar={<Icon name="pen" size={20} />} />}
      onOk={handleOk}
      okText={t('OK')}
      onCancel={onCancel}
    ...
    >
      <EditForm form={form} initialValues={initData}>
        <FormItem name={['name']} label={t('APP_STORE_FIELD_NAME')} help={t('LONG_NAME_DESC')}>
          <Input disabled />
        </FormItem>
        <FormItem label={t('APP_STORE_ALIAS')} help={t('APP_STORE_ALIAS_DESC')} name={['aliasName']}>
          <Input autoComplete="off" maxLength={63} />
        </FormItem>
        <FormItem name={['description']} label={t('APP_STORE_DESCRIPTION')} help={t('APP_STORE_DESCRIPTION_DESC')}>
