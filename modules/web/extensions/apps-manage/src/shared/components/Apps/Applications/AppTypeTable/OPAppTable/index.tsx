...
export function OPAppTable({
  ...
  const { mutate: mutateDelete, isLoading: isDeleting } = useAppDeleteMutation({
    onSuccess: () => {
      ...
      closeModal();
      notify.success(t('APP_STORE_DELETED_SUCCESSFUL'));
  ...
  return (
    <>
      <DataTable
        ...
        placeholder={t('APP_STORE_SEARCH_BY_NAME')}
        ...
        emptyOptions={{
          element: (
            <EmptyPlaceholder
              title={t('APP_STORE_NO_TEMPLATE_BASED_APP_FOUND')}
              description={t('APP_STORE_TEMPLATE_BASED_APP_EMPTY_DESC')}
