...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &v1alpha12.HelmRepo{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func (r *ReconcileHelmRepo) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.ObjectMeta.DeletionTimestamp.IsZero() {
		if instance.Status.State == "" {
			instance.Status.State = v1alpha12.RepoStateSyncing
	...
	if syncNow, after := needReSyncNow(copyInstance); syncNow {
		...
		if syncErr != nil {
			// failed
			state = append([]v1alpha12.HelmRepoSyncState{{
				State:    v1alpha12.RepoStateFailed,
				Message:  strings.ShortenString(syncErr.Error(), MessageLen),
				SyncTime: &now,
			}}, state...)
			copyInstance.Status.State = v1alpha12.RepoStateFailed
		} else {
			state = append([]v1alpha12.HelmRepoSyncState{{
				State:    v1alpha12.RepoStateSuccessful,
				SyncTime: &now,
			}}, state...)

			copyInstance.Status.Version = instance.Spec.Version
			copyInstance.Status.State = v1alpha12.RepoStateSuccessful
		}
...
func needReSyncNow(instance *v1alpha12.HelmRepo) (syncNow bool, after int) {
...
func (r *ReconcileHelmRepo) syncRepo(instance *v1alpha12.HelmRepo) error {
