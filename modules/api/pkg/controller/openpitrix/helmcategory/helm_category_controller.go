...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &v1alpha12.HelmCategory{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
	...
	err = c.Watch(&source.Kind{Type: &v1alpha12.HelmApplication{}}, &handler.Funcs{
		CreateFunc: func(event event.CreateEvent, limitingInterface workqueue.RateLimitingInterface) {
			app := event.Object.(*v1alpha12.HelmApplication)
			...
			if repoId == v1alpha12.AppStoreRepoId {
				ctgId := app.GetHelmCategoryId()
				if ctgId == "" {
					ctgId = v1alpha12.UncategorizedId
				}
		...
		UpdateFunc: func(updateEvent event.UpdateEvent, limitingInterface workqueue.RateLimitingInterface) {
			oldApp := updateEvent.ObjectOld.(*v1alpha12.HelmApplication)
			newApp := updateEvent.ObjectNew.(*v1alpha12.HelmApplication)
			...
			if repoId == v1alpha12.AppStoreRepoId {
				...
				if oldId == "" {
					oldId = v1alpha12.UncategorizedId
				}
			...
			if repoId == v1alpha12.AppStoreRepoId {
				...
				if newId == "" {
					newId = v1alpha12.UncategorizedId
				}
				if oldId != newId {
		...
		DeleteFunc: func(deleteEvent event.DeleteEvent, limitingInterface workqueue.RateLimitingInterface) {
			app := deleteEvent.Object.(*v1alpha12.HelmApplication)
			repoId := app.GetHelmRepoId()
			if repoId == v1alpha12.AppStoreRepoId {
				id := app.GetHelmCategoryId()
				if id == "" {
					id = v1alpha12.UncategorizedId
				}
	...
	go func() {
		...
		for range ticker.C {
			...
			err := reconcileObj.Get(context.TODO(), types.NamespacedName{Name: v1alpha12.UncategorizedId}, ctg)
			if err != nil && !errors.IsNotFound(err) {
				klog.Errorf("get helm category: %s failed, error: %s", v1alpha12.UncategorizedId, err)
			}
			if ctg.Name != "" {
				// category exists now
				return
			...
			ctg = &v1alpha12.HelmCategory{
				...
				Spec: v1alpha12.HelmCategorySpec{
					Description: v1alpha12.UncategorizedName,
					Name:        v1alpha12.UncategorizedName,
				},
			}
			err = reconcileObj.Create(context.TODO(), ctg)
			if err != nil {
				klog.Errorf("create helm category: %s failed, error: %s", v1alpha12.UncategorizedName, err)
			}
		}
	}()
	return nil
...
func (r *ReconcileHelmCategory) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	instance := &v1alpha12.HelmCategory{}
	err := r.Client.Get(context.TODO(), request.NamespacedName, instance)
	if err != nil {
		if errors.IsNotFound(err) {
			if request.Name == v1alpha12.UncategorizedId {
...
func (r *ReconcileHelmCategory) ensureUncategorizedCategory() error {
	ctg := &v1alpha12.HelmCategory{}
	err := r.Get(context.TODO(), types.NamespacedName{Name: v1alpha12.UncategorizedId}, ctg)
	if err != nil && !errors.IsNotFound(err) {
		return err
	}

	ctg.Name = v1alpha12.UncategorizedId
	ctg.Spec.Name = v1alpha12.UncategorizedName
	ctg.Spec.Description = v1alpha12.UncategorizedName
...
func (r *ReconcileHelmCategory) countApplications(id string) (int, error) {
	list := v1alpha12.HelmApplicationList{}
	var err error
	err = r.List(context.TODO(), &list, client.MatchingLabels{
		constants.CategoryIdLabelKey:  id,
		constants.ChartRepoIdLabelKey: v1alpha12.AppStoreRepoId,
	})
	...
	for _, app := range list.Items {
		if app.Status.State == v1alpha12.StateActive {
			count += 1
		}
	}
...
func (r *ReconcileHelmCategory) updateUncategorizedApplicationLabels(app *v1alpha12.HelmApplication) error {
	if app == nil {
		return nil
	}
	if app.GetHelmRepoId() == v1alpha12.AppStoreRepoId && app.GetHelmCategoryId() == "" {
		appCopy := app.DeepCopy()
		appCopy.Labels[constants.CategoryIdLabelKey] = v1alpha12.UncategorizedId
