...
import (
	...
	v1alpha12 "kubesphere.io/openpitrix/pkg/api/cluster/v1alpha1"
...
func (r *ReconcileHelmRelease) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	instance := &v1alpha13.HelmRelease{}
	...
	if instance.Status.State == "" {
		instance.Status.State = v1alpha13.HelmStatusCreating
			...
			if r.MultiClusterEnable && clusterName != "" {
				...
				if !r.clusterClients.IsHostCluster(clusterInfo) {
					...
					instance.OwnerReferences = append(instance.OwnerReferences, metav1.OwnerReference{
						...
						Kind:       v1alpha12.ResourceKindCluster,
...
func (r *ReconcileHelmRelease) reconcile(instance *v1alpha13.HelmRelease) (reconcile.Result, error) {

	var err error
	switch instance.Status.State {
	case v1alpha13.HelmStatusDeleting, v1alpha13.HelmStatusFailed:
		// no operation
		return reconcile.Result{}, nil
	case v1alpha13.HelmStatusActive:
		...
		if instance.Status.Version != instance.Spec.Version {
			instance.Status.State = v1alpha13.HelmStatusUpgrading
...
func (r *ReconcileHelmRelease) checkReleaseJobStatus(instance *v1alpha13.HelmRelease) (reconcile.Result, error) {
	...
	if latestJobCondition.Type == batchv1.JobComplete && latestJobCondition.Status == corev1.ConditionTrue {
		...
		err = r.updateStatus(instance, v1alpha13.HelmStatusActive, "")

	} else if latestJobCondition.Type == batchv1.JobFailed && latestJobCondition.Status == corev1.ConditionTrue {
		delete(instance.Annotations, "kubesphere.io/jobName")
		err = r.updateStatus(instance, v1alpha13.HelmStatusFailed, fmt.Sprintf("helm executor job failed: %s", latestJobCondition.Message))
	} else {
...
func rlsBackoffKey(rls *v1alpha13.HelmRelease) string {
	return rls.Name
}

// doCheck check whether helm release's resources are ready or not.
func (r *ReconcileHelmRelease) doCheck(rls *v1alpha13.HelmRelease) (retryAfter time.Duration, err error) {
	...
	if err != nil {
		...
		err := r.updateStatus(rls, v1alpha13.HelmStatusActive, "")
...
func (r *ReconcileHelmRelease) checkReleaseIsReady(rls *v1alpha13.HelmRelease) (retryAfter time.Duration, err error) {
	backoffKey := rlsBackoffKey(rls)
	now := time.Now()
...
func (r *ReconcileHelmRelease) updateStatus(rls *v1alpha13.HelmRelease, currentState, msg string) error {
	now := metav1.Now()
	var deployStatus v1alpha13.HelmReleaseDeployStatus
	rls.Status.Message = stringutils.ShortenString(msg, v1alpha13.MsgLen)

	deployStatus.Message = stringutils.ShortenString(msg, v1alpha13.MsgLen)
	deployStatus.State = currentState
	deployStatus.Time = now

	if rls.Status.State != currentState &&
		(currentState == v1alpha13.HelmStatusCreated || currentState == v1alpha13.HelmStatusUpgraded) {
	...
	rls.Status.DeployStatus = append([]v1alpha13.HelmReleaseDeployStatus{deployStatus}, rls.Status.DeployStatus...)
...
func (r *ReconcileHelmRelease) createOrUpgradeHelmRelease(rls *v1alpha13.HelmRelease, upgrade bool) (reconcile.Result, error) {
	...
	hw, err = helm.NewExecutor(clusterConfig, rls.GetRlsNamespace(), rls.Spec.Name,
		...
		helm.SetLabels(map[string]string{v1alpha13.ApplicationInstance: rls.GetTrueName()}))
	if err != nil {
	...
	if upgrade {
		...
		currentState = v1alpha13.HelmStatusUpgraded
	} else {
		...
		currentState = v1alpha13.HelmStatusCreated
	}

	var msg string
	if err != nil {
		// install or upgrade failed
		currentState = v1alpha13.HelmStatusFailed
		msg = err.Error()
	}
...
func (r *ReconcileHelmRelease) uninstallHelmRelease(rls *v1alpha13.HelmRelease) error {

	if rls.Status.State != v1alpha13.HelmStatusDeleting {
		rls.Status.State = v1alpha13.HelmStatusDeleting
