...
func (r *ReconcileHelmApplicationVersion) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	appVersion := &v1alpha12.HelmApplicationVersion{}
	...
	if appVersion.Status.State == v1alpha12.StateActive {
		...
		if appVersion.GetHelmRepoId() == "" {
			...
			instanceCopy.Labels[constants.ChartRepoIdLabelKey] = v1alpha12.AppStoreRepoId
	...
	} else if appVersion.Status.State == v1alpha12.StateSuspended {
		return reconcile.Result{}, updateHelmApplicationStatus(r.Client, appVersion.GetHelmApplicationId(), true)
	}
...
func updateHelmApplicationStatus(c client.Client, appId string, inAppStore bool) error {
	...
	if inAppStore {
		...
		err = c.Get(context.TODO(), types.NamespacedName{Name: fmt.Sprintf("%s%s", appId, v1alpha12.HelmApplicationAppStoreSuffix)}, &app)
	} else {
	...
	var versions v1alpha12.HelmApplicationVersionList
...
func (r *ReconcileHelmApplicationVersion) updateStatus(appVersion *v1alpha12.HelmApplicationVersion) error {
	appVersion.Status = v1alpha12.HelmApplicationVersionStatus{
...
func getLatestVersionName(versions v1alpha12.HelmApplicationVersionList, inAppStore bool) string {
	...
	for _, version := range versions.Items {
		...
		if version.DeletionTimestamp != nil || (inAppStore && version.Status.State != v1alpha12.StateActive) {
			continue
		}
...
func mergeApplicationVersionState(versions v1alpha12.HelmApplicationVersionList) string {
	...
	if states[v1alpha12.StateActive] > 0 {
		return v1alpha12.StateActive
	}

	// All appVersion is draft, the helm application is draft
	if states[v1alpha12.StateDraft] == len(versions.Items) {
		return v1alpha12.StateDraft
	}

	// No active appVersion or draft appVersion, then the app state is suspended
	if states[v1alpha12.StateSuspended] > 0 {
		return v1alpha12.StateSuspended
	}
...
func (r *ReconcileHelmApplicationVersion) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha12.HelmApplicationVersion{}).
		Complete(r)
}
