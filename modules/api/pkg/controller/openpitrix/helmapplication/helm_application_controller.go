	...
	if app.DeletionTimestamp == nil {
		...
		if !inAppStore(app) {
			...
			if app.Status.State == v1alpha12.StateActive ||
				app.Status.State == v1alpha12.StateSuspended {
		...
		return reconcile.Result{}, updateHelmApplicationStatus(r.Client, strings.TrimSuffix(app.Name, v1alpha12.HelmApplicationAppStoreSuffix), inAppStore(app))
	} else {
...
func (r *ReconcileHelmApplication) deleteAppCopyInAppStore(ctx context.Context, name string) error {
	appInStore := &v1alpha12.HelmApplication{}
	err := r.Client.Get(ctx, types.NamespacedName{Name: fmt.Sprintf("%s%s", name, v1alpha12.HelmApplicationAppStoreSuffix)}, appInStore)
	if err != nil {
		if !apierrors.IsNotFound(err) {
			return err
		}
...
func (r *ReconcileHelmApplication) createAppCopyInAppStore(ctx context.Context, originApp *v1alpha12.HelmApplication) error {
	name := fmt.Sprintf("%s%s", originApp.Name, v1alpha12.HelmApplicationAppStoreSuffix)
	...
	if app.Name == "" {
		...
		labels[constants.ChartRepoIdLabelKey] = v1alpha12.AppStoreRepoId

		// assign a default category to app
		if labels[constants.CategoryIdLabelKey] == "" {
			labels[constants.CategoryIdLabelKey] = v1alpha12.UncategorizedId
		}
		// record the original workspace
		labels[v1alpha12.OriginWorkspaceLabelKey] = originApp.GetWorkspace()
...
func (r *ReconcileHelmApplication) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha12.HelmApplication{}).Complete(r)
}

func inAppStore(app *v1alpha12.HelmApplication) bool {
	return strings.HasSuffix(app.Name, v1alpha12.HelmApplicationAppStoreSuffix)
}
...
func (r *ReconcileHelmApplication) cleanupDanglingApp(ctx context.Context, app *v1alpha12.HelmApplication) error {
	if app.Annotations != nil && app.Annotations[constants.DanglingAppCleanupKey] == constants.CleanupDanglingAppOngoing {
		...
		if app.Status.State != v1alpha12.StateActive && app.Status.State != v1alpha12.StateSuspended {
		...
		var appVersions v1alpha12.HelmApplicationVersionList
		...
		for _, version := range appVersions.Items {
			if version.Status.State != v1alpha12.StateActive && version.Status.State != v1alpha12.StateSuspended {
		...
		var appInStore v1alpha12.HelmApplication
		err = r.Get(ctx,
			types.NamespacedName{Name: fmt.Sprintf("%s%s", app.GetHelmApplicationId(), v1alpha12.HelmApplicationAppStoreSuffix)}, &appInStore)
		if err != nil {
			if !apierrors.IsNotFound(err) {
				return err
			}
