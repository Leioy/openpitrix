...
type HelmApplicationLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha12.HelmApplication, err error)
	// Get retrieves the HelmApplication from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha12.HelmApplication, error)
...
// List lists all HelmApplications in the indexer.
func (s *helmApplicationLister) List(selector labels.Selector) (ret []*v1alpha12.HelmApplication, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha12.HelmApplication))
	})
	return ret, err
}

// Get retrieves the HelmApplication from the index for a given name.
func (s *helmApplicationLister) Get(name string) (*v1alpha12.HelmApplication, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha12.Resource("helmapplication"), name)
	}
	return obj.(*v1alpha12.HelmApplication), nil
}
