...
type HelmRepoLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha12.HelmRepo, err error)
	// Get retrieves the HelmRepo from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha12.HelmRepo, error)
...
// List lists all HelmRepos in the indexer.
func (s *helmRepoLister) List(selector labels.Selector) (ret []*v1alpha12.HelmRepo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha12.HelmRepo))
	})
	return ret, err
}

// Get retrieves the HelmRepo from the index for a given name.
func (s *helmRepoLister) Get(name string) (*v1alpha12.HelmRepo, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha12.Resource("helmrepo"), name)
	}
	return obj.(*v1alpha12.HelmRepo), nil
}
