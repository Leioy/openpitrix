...
type HelmCategoryLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha12.HelmCategory, err error)
	// Get retrieves the HelmCategory from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha12.HelmCategory, error)
...
// List lists all HelmCategories in the indexer.
func (s *helmCategoryLister) List(selector labels.Selector) (ret []*v1alpha12.HelmCategory, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha12.HelmCategory))
	})
	return ret, err
}

// Get retrieves the HelmCategory from the index for a given name.
func (s *helmCategoryLister) Get(name string) (*v1alpha12.HelmCategory, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha12.Resource("helmcategory"), name)
	}
	return obj.(*v1alpha12.HelmCategory), nil
}
