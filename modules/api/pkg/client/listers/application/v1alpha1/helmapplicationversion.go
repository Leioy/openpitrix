...
type HelmApplicationVersionLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha12.HelmApplicationVersion, err error)
	// Get retrieves the HelmApplicationVersion from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha12.HelmApplicationVersion, error)
...
// List lists all HelmApplicationVersions in the indexer.
func (s *helmApplicationVersionLister) List(selector labels.Selector) (ret []*v1alpha12.HelmApplicationVersion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha12.HelmApplicationVersion))
	})
	return ret, err
}

// Get retrieves the HelmApplicationVersion from the index for a given name.
func (s *helmApplicationVersionLister) Get(name string) (*v1alpha12.HelmApplicationVersion, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha12.Resource("helmapplicationversion"), name)
	}
	return obj.(*v1alpha12.HelmApplicationVersion), nil
}
