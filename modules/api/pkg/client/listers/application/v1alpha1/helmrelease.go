...
type HelmReleaseLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha12.HelmRelease, err error)
	// Get retrieves the HelmRelease from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha12.HelmRelease, error)
...
// List lists all HelmReleases in the indexer.
func (s *helmReleaseLister) List(selector labels.Selector) (ret []*v1alpha12.HelmRelease, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha12.HelmRelease))
	})
	return ret, err
}

// Get retrieves the HelmRelease from the index for a given name.
func (s *helmReleaseLister) Get(name string) (*v1alpha12.HelmRelease, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha12.Resource("helmrelease"), name)
	}
	return obj.(*v1alpha12.HelmRelease), nil
}
