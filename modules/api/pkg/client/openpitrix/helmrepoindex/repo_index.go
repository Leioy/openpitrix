...
func LoadRepoIndex(ctx context.Context, u string, cred *v1alpha12.HelmRepoCredential) (*helmrepo.IndexFile, error) {
...
func MergeRepoIndex(repo *v1alpha12.HelmRepo, index *helmrepo.IndexFile, existsSavedIndex *SavedIndex) *SavedIndex {
	saved := &SavedIndex{}
	if index == nil {
	...
	for name, versions := range index.Entries {
		...
		if application, exists := saved.Applications[name]; !exists {
			...
			if IsBuiltInRepo(repo.Name) {
				application.ApplicationId = fmt.Sprintf("%s%s-%s", v1alpha12.HelmApplicationIdPrefix, repo.Name, name)
			} else {
				application.ApplicationId = idutils.GetUuid36(v1alpha12.HelmApplicationIdPrefix)
			}
...
func (i *SavedIndex) GetApplicationVersion(appId, versionId string) *v1alpha12.HelmApplicationVersion {
...
// The apps in the system repo have been audited by the admin, so the name of the charts should not include malformed text.
// Then we can add the name string to the labels of the k8s object.
func generateAppVersionId(repo *v1alpha12.HelmRepo, chartName, version string) string {
	if IsBuiltInRepo(repo.Name) {
		return fmt.Sprintf("%s%s-%s-%s", v1alpha12.HelmApplicationIdPrefix, repo.Name, chartName, version)
	} else {
		return idutils.GetUuid36(v1alpha12.HelmApplicationVersionIdPrefix)
	}
...
func IsBuiltInRepo(repoName string) bool {
	return strings.HasPrefix(repoName, v1alpha12.BuiltinRepoPrefix)
}
