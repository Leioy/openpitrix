...
import (
	...
	helmrepoindex2 "kubesphere.io/openpitrix/pkg/client/openpitrix/helmrepoindex"
...
func NewReposCache() ReposCache {
	return &cachedRepos{
		...
		repos:                 map[string]*v1alpha12.HelmRepo{},
		apps:                  map[string]*v1alpha12.HelmApplication{},
		versions:              map[string]*v1alpha12.HelmApplicationVersion{},
		builtinCategoryCounts: map[string]int{},
	}
}

type ReposCache interface {
	AddRepo(repo *v1alpha12.HelmRepo) error
	DeleteRepo(repo *v1alpha12.HelmRepo) error
	UpdateRepo(old, new *v1alpha12.HelmRepo) error

	GetApplication(string) (*v1alpha12.HelmApplication, bool)
	GetAppVersion(string) (*v1alpha12.HelmApplicationVersion, bool, error)
	GetAppVersionWithData(string) (*v1alpha12.HelmApplicationVersion, bool, error)

	ListAppVersionsByAppId(appId string) (ret []*v1alpha12.HelmApplicationVersion, exists bool)
	ListApplicationsInRepo(repoId string) (ret []*v1alpha12.HelmApplication, exists bool)
	ListApplicationsInBuiltinRepo(selector labels.Selector) (ret []*v1alpha12.HelmApplication, exists bool)
...
func (c *cachedRepos) deleteRepo(repo *v1alpha12.HelmRepo) {
	if len(repo.Status.Data) == 0 {
		return
	}

	index, err := helmrepoindex2.ByteArrayToSavedIndex([]byte(repo.Status.Data))
	if err != nil {
	...
	for _, app := range index.Applications {
		...
		if helmrepoindex2.IsBuiltInRepo(repo.Name) {
...
func (c *cachedRepos) DeleteRepo(repo *v1alpha12.HelmRepo) error {
	c.Lock()
	defer c.Unlock()
...
func (c *cachedRepos) translateCategoryNameToId(ctgName string) string {
	if c.indexerOfHelmCtg == nil || ctgName == "" {
		return v1alpha12.UncategorizedId
	}

	if items, err := c.indexerOfHelmCtg.ByIndex(CategoryIndexer, ctgName); len(items) == 0 || err != nil {
		return v1alpha12.UncategorizedId
	} else {
		obj, _ := items[0].(*v1alpha12.HelmCategory)
		return obj.Name
	}
}

func (c *cachedRepos) GetApplication(appId string) (app *v1alpha12.HelmApplication, exists bool) {
	c.RLock()
	defer c.RUnlock()
...
func (c *cachedRepos) UpdateRepo(old, new *v1alpha12.HelmRepo) error {
...
func (c *cachedRepos) AddRepo(repo *v1alpha12.HelmRepo) error {
	c.Lock()
	defer c.Unlock()
...
func (c *cachedRepos) addRepo(repo *v1alpha12.HelmRepo, builtin bool) error {
	if len(repo.Status.Data) == 0 {
		return nil
	}
	index, err := helmrepoindex2.ByteArrayToSavedIndex([]byte(repo.Status.Data))
	...
	for key, app := range index.Applications {
		...
		helmApp := v1alpha12.HelmApplication{
			...
			Status: v1alpha12.HelmApplicationStatus{
		...
		for _, chartVersion := range app.Charts {
			chartsCount += 1
			hvw := helmrepoindex2.HelmVersionWrapper{ChartVersion: &chartVersion.ChartVersion}
			appVerName = chartVersion.ApplicationVersionId
			version := &v1alpha12.HelmApplicationVersion{
				ObjectMeta: metav1.ObjectMeta{
					Name:        appVerName,
...
func (c *cachedRepos) ListApplicationsInRepo(repoId string) (ret []*v1alpha12.HelmApplication, exists bool) {
	c.RLock()
	defer c.RUnlock()
	...
	if repo, exists := c.repos[repoId]; !exists {
		...
		ret = make([]*v1alpha12.HelmApplication, 0, 10)
...
func (c *cachedRepos) ListApplicationsInBuiltinRepo(selector labels.Selector) (ret []*v1alpha12.HelmApplication, exists bool) {
	c.RLock()
	defer c.RUnlock()

	ret = make([]*v1alpha12.HelmApplication, 0, 20)
	for _, app := range c.apps {
		if strings.HasPrefix(app.GetHelmRepoId(), v1alpha12.BuiltinRepoPrefix) {
...
func (c *cachedRepos) ListAppVersionsByAppId(appId string) (ret []*v1alpha12.HelmApplicationVersion, exists bool) {
	c.RLock()
	defer c.RUnlock()
	...
	ret = make([]*v1alpha12.HelmApplicationVersion, 0, 10)
...
func (c *cachedRepos) getAppVersion(versionId string, withData bool) (ret *v1alpha12.HelmApplicationVersion, exists bool, err error) {
	c.RLock()
	if version, exists := c.versions[versionId]; exists {
		...
		if withData {
			...
			buf, err := helmrepoindex2.LoadChart(context.TODO(), url, &repo.Spec.Credential)
			if err != nil {
...
func (c *cachedRepos) GetAppVersion(versionId string) (ret *v1alpha12.HelmApplicationVersion, exists bool, err error) {
	return c.getAppVersion(versionId, false)
}

func (c *cachedRepos) GetAppVersionWithData(versionId string) (ret *v1alpha12.HelmApplicationVersion, exists bool, err error) {
