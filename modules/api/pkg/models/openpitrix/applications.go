...
import (
	...
	"errors"
	"fmt"
	v1alpha14 "kubesphere.io/openpitrix/pkg/api/application/v1alpha1"
	"kubesphere.io/openpitrix/pkg/apiserver/query"
	"kubesphere.io/openpitrix/pkg/client/clientset/versioned"
	v1alpha15 "kubesphere.io/openpitrix/pkg/client/clientset/versioned/typed/application/v1alpha1"
	"kubesphere.io/openpitrix/pkg/client/informers/externalversions"
	v1alpha12 "kubesphere.io/openpitrix/pkg/client/listers/application/v1alpha1"
...
type applicationOperator struct {
	...
	appClient        v1alpha15.HelmApplicationInterface
	appVersionClient v1alpha15.HelmApplicationVersionInterface

	appLister     v1alpha12.HelmApplicationLister
	versionLister v1alpha12.HelmApplicationVersionLister

	repoLister v1alpha12.HelmRepoLister
	ctgLister  v1alpha12.HelmCategoryLister
	rlsLister  v1alpha12.HelmReleaseLister
...
func (c *applicationOperator) createApp(app *v1alpha14.HelmApplication, iconData []byte) (*v1alpha14.HelmApplication, error) {
	...
	if len(iconData) != 0 {
		...
		iconId := idutils.GetUuid(v1alpha14.HelmAttachmentPrefix)
...
func (c *applicationOperator) getHelmAppByName(workspace, name string) (*v1alpha14.HelmApplication, error) {
...
func (c *applicationOperator) DoAppAction(appId string, request *ActionRequest) error {
	...
	if repoId, exist := app.Labels[constants.ChartRepoIdLabelKey]; exist && repoId != v1alpha14.AppStoreRepoId {
		return apierrors.NewForbidden(v1alpha14.Resource(v1alpha14.ResourcePluralHelmApplication), app.Name, errors.New("application is immutable"))
	}

	var filterState string
	switch request.Action {
	case ActionSuspend:
		if app.Status.State != v1alpha14.StateActive {
			err = actionNotSupport
		}
		filterState = v1alpha14.StateActive
	case ActionRecover:
		if app.Status.State != v1alpha14.StateSuspended {
			err = actionNotSupport
		}
		filterState = v1alpha14.StateSuspended
	default:
		err = actionNotSupport
	}
	if err != nil {
	...
	var versions []*v1alpha14.HelmApplicationVersion
	ls := map[string]string{
...
func (c *applicationOperator) CreateApp(req *CreateAppRequest) (*CreateAppResponse, error) {
	...
	name := idutils.GetUuid36(v1alpha14.HelmApplicationIdPrefix)
	helmApp := &v1alpha14.HelmApplication{
		...
		Spec: v1alpha14.HelmApplicationSpec{
			Name:        req.Name,
			Description: stringutils.ShortenString(chrt.GetDescription(), v1alpha14.MsgLen),
			Icon:        stringutils.ShortenString(chrt.GetIcon(), v1alpha14.MsgLen),
		},
	}
...
func (c *applicationOperator) DeleteApp(id string) error {
	...
	id = fmt.Sprintf("%s%s", id, v1alpha14.HelmApplicationAppStoreSuffix)
...
func (c *applicationOperator) deleteAppAttachment(app *v1alpha14.HelmApplication) {
	if app.Spec.Icon != "" {
...
func (c *applicationOperator) modifyAppAttachment(app *v1alpha14.HelmApplication, request *ModifyAppRequest) (add string, err error) {
	if request.Type == nil {
		return "", nil
	}
	switch *request.Type {
	case v1alpha14.AttachmentTypeScreenshot:
	...
	case v1alpha14.AttachmentTypeIcon: // modify app icon
		// delete old icon
		if app.Spec.Icon != "" {
...
func (c *applicationOperator) appAttachmentDiff(old, newApp *v1alpha14.HelmApplication) (added, deleted []string) {
...
func (c *applicationOperator) DescribeApp(id string) (*App, error) {
	var helmApp *v1alpha14.HelmApplication
	var ctg *v1alpha14.HelmCategory
	var err error
...
func (c *applicationOperator) listApps(conditions *params.Conditions) (ret []*v1alpha14.HelmApplication, err error) {
	repoId := conditions.Match[RepoId]
	if repoId != "" && repoId != v1alpha14.AppStoreRepoId {
	...
	} else if repoId == v1alpha14.AppStoreRepoId {
		// List apps in the app-store and built-in repo
		if c.backingStoreClient == nil {
			return []*v1alpha14.HelmApplication{}, nil
		}
...
func (c *applicationOperator) getHelmApplication(appId string) (*v1alpha14.HelmApplication, error) {
