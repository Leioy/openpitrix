...
func (c *applicationOperator) GetAppVersionPackage(appId, versionId string) (*GetAppVersionPackageResponse, error) {
	var version *v1alpha12.HelmApplicationVersion
	var err error
...
func (c *applicationOperator) ModifyAppVersion(id string, request *ModifyAppVersionRequest) error {
	...
	if repoId, exists := version.Labels[constants.ChartRepoIdLabelKey]; exists && repoId != v1alpha12.AppStoreRepoId {
		return apierrors.NewForbidden(v1alpha12.Resource(v1alpha12.ResourcePluralHelmApplicationVersion), version.Name, errors.New("version is immutable"))
	}
	...
	if len(request.Package) > 0 {
		if version.Status.State != v1alpha12.StateDraft {
			return actionNotPermitted
		}
		...
		spec.Description = stringutils.ShortenString(chart.GetDescription(), v1alpha12.MsgLen)
		...
		if request.Description != nil && *request.Description != "" {
			spec.Description = stringutils.ShortenString(*request.Description, v1alpha12.MsgLen)
		}
	}
...
func (c *applicationOperator) ListAppVersionAudits(conditions *params.Conditions, orderBy string, reverse bool, limit, offset int) (*models.PageableResponse, error) {
	...
	var versions []*v1alpha12.HelmApplicationVersion
	var err error
	if versionId == "" {
		...
		versions = []*v1alpha12.HelmApplicationVersion{version}
	}
...
func (c *applicationOperator) DoAppVersionAction(versionId string, request *ActionRequest) error {
	...
	case ActionCancel:
		if version.Status.State != v1alpha12.StateSubmitted {
		}
		state = v1alpha12.StateDraft
		audit.State = v1alpha12.StateDraft
	case ActionPass:
		if version.Status.State != v1alpha12.StateSubmitted {

		}
		state = v1alpha12.StatePassed
		audit.State = v1alpha12.StatePassed
	case ActionRecover:
		if version.Status.State != v1alpha12.StateSuspended {

		}
		state = v1alpha12.StateActive
		audit.State = v1alpha12.StateActive
	case ActionReject:
		if version.Status.State != v1alpha12.StateSubmitted {
			// todo check status
		}
		state = v1alpha12.StateRejected
		audit.State = v1alpha12.StateRejected
	case ActionSubmit:
		if version.Status.State != v1alpha12.StateDraft {
			// todo check status
		}
		state = v1alpha12.StateSubmitted
		audit.State = v1alpha12.StateSubmitted
	case ActionSuspend:
		if version.Status.State != v1alpha12.StateActive {
			// todo check status
		}
		state = v1alpha12.StateSuspended
		audit.State = v1alpha12.StateSuspended
	case ActionRelease:
		// release to app store
		if version.Status.State != v1alpha12.StatePassed {
			// todo check status
		}
		state = v1alpha12.StateActive
		audit.State = v1alpha12.StateActive
	...
	if request.Action == ActionRelease || request.Action == ActionRecover {
		...
		appInStore, err := c.appLister.Get(fmt.Sprintf("%s%s", version.GetHelmApplicationId(), v1alpha12.HelmApplicationAppStoreSuffix))
		if err != nil {
			if apierrors.IsNotFound(err) {
...
func (c *applicationOperator) getAppVersionByVersionName(appId, verName string) (*v1alpha12.HelmApplicationVersion, error) {
	ls := map[string]string{
		constants.ChartApplicationIdLabelKey: appId,
	}
...
func (c *applicationOperator) createApplicationVersion(ver *v1alpha12.HelmApplicationVersion) (*v1alpha12.HelmApplicationVersion, error) {
...
func (c *applicationOperator) updateAppVersionStatus(version *v1alpha12.HelmApplicationVersion, state string, status *v1alpha12.Audit) (*v1alpha12.HelmApplicationVersion, error) {
	version.Status.State = state

	states := append([]v1alpha12.Audit{*status}, version.Status.Audit...)
	if len(version.Status.Audit) >= v1alpha12.HelmRepoSyncStateLen {
		// strip the last item
		states = states[:v1alpha12.HelmRepoSyncStateLen:v1alpha12.HelmRepoSyncStateLen]
	}
...
func (c *applicationOperator) getAppVersionByVersionIdWithData(versionId string) (*v1alpha12.HelmApplicationVersion, error) {
...
func (c *applicationOperator) getAppVersionsByAppId(appId string) (ret []*v1alpha12.HelmApplicationVersion, err error) {
...
func (c *applicationOperator) getAppVersion(id string) (ret *v1alpha12.HelmApplicationVersion, err error) {
