...
import (
	...
	"errors"
	"fmt"
	v1alpha13 "kubesphere.io/openpitrix/pkg/api/application/v1alpha1"
	...
	"kubesphere.io/openpitrix/pkg/utils/resourceparse"
...
type ReleaseInterface interface {
	...
	CreateApplication(workspace, clusterName, namespace string, request CreateClusterRequest) (*v1alpha13.HelmRelease, error)
	ModifyApplication(request ModifyClusterAttributesRequest) error
	DeleteApplication(workspace, clusterName, namespace, id string) (*v1alpha13.HelmRelease, error)
	UpgradeApplication(request UpgradeClusterRequest) error
	DetailsApplication(workspace, clusterName, namespace, applicationId string) (*v1alpha13.HelmRelease, error)
}
...
type releaseOperator struct {
	...
	appVersionLister v1alpha12.HelmApplicationVersionLister
...
func (c *releaseOperator) UpgradeApplication(request UpgradeClusterRequest) error {
	...
	case v1alpha13.StateActive, v1alpha13.HelmStatusUpgraded, v1alpha13.HelmStatusCreated:
		// no operation
	default:
...
func (c *releaseOperator) CreateApplication(workspace, clusterName, namespace string, request CreateClusterRequest) (*v1alpha13.HelmRelease, error) {
	version, err := c.getAppVersion("", request.VersionId)
	...
	rls := &v1alpha13.HelmRelease{
		ObjectMeta: metav1.ObjectMeta{
			Name: idutils.GetUuid36(v1alpha13.HelmReleasePrefix),
			Annotations: map[string]string{
				constants.CreatorAnnotationKey: request.Username,
			},
			Labels: map[string]string{
				constants.ChartApplicationVersionIdLabelKey: request.VersionId,
				constants.ChartApplicationIdLabelKey:        strings.TrimSuffix(request.AppId, v1alpha13.HelmApplicationAppStoreSuffix),
				constants.WorkspaceLabelKey:                 request.Workspace,
				constants.NamespaceLabelKey:                 namespace,
			},
		},
		Spec: v1alpha13.HelmReleaseSpec{
			Name:                 request.Name,
			Description:          stringutils.ShortenString(request.Description, v1alpha13.MsgLen),
			Version:              1,
			Values:               strfmt.Base64(request.Conf),
			ApplicationId:        strings.TrimSuffix(request.AppId, v1alpha13.HelmApplicationAppStoreSuffix),
...
func (c *releaseOperator) ModifyApplication(request ModifyClusterAttributesRequest) error {
	...
	rlsCopy.Spec.Description = stringutils.ShortenString(strings.TrimSpace(*request.Description), v1alpha13.MsgLen)
...
func (c *releaseOperator) ListApplications(workspace, clusterName, namespace string, conditions *params.Conditions, limit, offset int, orderBy string, reverse bool) (*models.PageableResponse, error) {
	...
	if appId != "" {
		ls[constants.ChartApplicationIdLabelKey] = strings.TrimSuffix(appId, v1alpha13.HelmApplicationAppStoreSuffix)
	}
	...
	if versionId == "" && strings.HasSuffix(appId, v1alpha13.HelmApplicationAppStoreSuffix) {
		stripId := strings.TrimSuffix(appId, v1alpha13.HelmApplicationAppStoreSuffix)
		...
		versions = filterAppVersionByState(versions, []string{v1alpha13.StateActive, v1alpha13.StateSuspended})
		versionMap := make(map[string]*v1alpha13.HelmApplicationVersion)
...
func (c *releaseOperator) DeleteApplication(workspace, clusterName, namespace, id string) (*v1alpha13.HelmRelease, error) {
...
func (c *releaseOperator) getAppVersion(repoId, id string) (ret *v1alpha13.HelmApplicationVersion, err error) {
	if ver, exists, _ := c.cachedRepos.GetAppVersion(id); exists {
		return ver, nil
	}

	if repoId != "" && repoId != v1alpha13.AppStoreRepoId {
...
// get app version from repo and helm application
func (c *releaseOperator) getAppVersionWithData(repoId, id string) (ret *v1alpha13.HelmApplicationVersion, err error) {
...
// get release details
func (c *releaseOperator) DetailsApplication(workspace, clusterName, namespace, applicationId string) (*v1alpha13.HelmRelease, error) {

	rls, err := c.rlsLister.Get(applicationId)
	name := rls.Annotations[v1alpha13.JobName]
	...
	if len(pods.Items) > 0 {
		rls.Annotations[v1alpha13.PodName] = pods.Items[0].Name
	}
