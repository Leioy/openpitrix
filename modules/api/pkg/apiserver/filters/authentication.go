...
import (
	"errors"
	"fmt"
	request2 "kubesphere.io/openpitrix/pkg/apiserver/request"
	"net/http"
...
func WithAuthentication(handler http.Handler, authRequest authenticator.Request) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil || !ok {
			ctx := req.Context()
			requestInfo, found := request2.RequestInfoFrom(ctx)
			if !found {
		...
		req = req.WithContext(request2.WithUser(req.Context(), resp.User))
